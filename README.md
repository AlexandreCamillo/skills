...
A pair of composable skills that turn "functional" UIs into genuinely spectacular ones. `visual-qa` audits a running app against a concrete 9-dimension design rubric and writes a structured report. `visual-refine` takes that report and drives spec → plan → execute → verify cycles until the rubric is satisfied, then runs a refactor + anti-regression pass. Neither skill ever commits on your behalf.

## How it works

It starts from the moment you point your coding agent at a running app and say `/visual-qa tela de login` (or just `/visual-qa` for the whole app).
...
Clone or download this repository, then copy the two skill directories into your Claude skills directory:

```bash
git clone https://github.com/<your-fork>/skills.git ~/projects/skills
...
A lightweight health-check script is included at `scripts/verify-visual-skills.sh` (in projects that have adopted the wrappers). It confirms that both `SKILL.md` files exist, parse as YAML, contain the required `<HARD-GATE>` and `digraph` markers, reference every sibling file, and that `design-principles.md` is byte-identical in both skills.
...
These skills are small and opinionated. If you have an improvement, the workflow is the same you'd use for any superpowers skill:
